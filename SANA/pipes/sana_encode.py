...
class SanaPipelineImg2Img(DiffusionPipeline):
    r"""
    ...
    def prepare_img_latents(self, image, timestep, batch_size, num_images_per_prompt, dtype, device, generator=None):
    ...
    def __call__(
        ...
        if not vaeonly:
            if not textonly or orig:
                #prompt_embeds = torch.load("/home/mohammad/pipelines/embds.pt",weights_only=True)
                #prompt_attention_mask = torch.load("/home/mohammad/pipelines/msk.pt",weights_only=True)
            ...
            if textonly and not orig:
                #torch.save(prompt_embeds,"/home/mohammad/pipelines/embds.pt")
                #print(prompt_attention_mask)
                #print(type(prompt_attention_mask),"\n",prompt_attention_mask.shape[-1])
                #torch.save(prompt_attention_mask,"/home/mohammad/pipelines/msk.pt")
                embeds = {
                    "prompt_embeds":prompt_embeds,
                    "prompt_attention_mask":prompt_attention_mask,
                    "negative_prompt_embeds":negative_prompt_embeds,
            ...
            latents = self.prepare_img_latents(
                image,
                latent_timestep,
                batch_size,
